...
def render_library() -> str:
    ...
    # Switching mode or stage is navigation, so it cancels a pending
    # new-lesson request rather than leaving it stuck on.
    context = (current_mode(), stage)
    if st.session_state.get("_nav_context") not in (None, context):
        st.session_state["show_new"] = False
    st.session_state["_nav_context"] = context
    ...
    if not rows:
        # Nothing in this stage, so there is nothing to open — but do NOT
        # persist show_new here. Doing so used to strand the app on the
        # new-lesson screen: the flag survived switching back to a stage that
        # did have lessons, and the only thing that cleared it was the lesson
        # dropdown's on_change, which never fires when the lesson you want is
        # already the selected value.
        st.sidebar.caption("Nothing approved yet." if stage == STAGE_DONE
                           else "Nothing in progress.")
        return NEW_LESSON

    if st.session_state.get("show_new"):
        # An unconditional way back, which the dropdown alone cannot provide.
        st.sidebar.button("←  Back to my lessons", use_container_width=True,
                          on_click=_stop_new)
...
def render_new_evaluation() -> None:
    ...
    # Mark the folder as an evaluation before anything can fail. Marking it only
    # after a successful parse left orphan folders: they exist on disk and show as
    # in progress, but Evaluate mode filters on evaluation.json, so nothing could
    # reach them. The item count is filled in below once the items parse.
    eval_mod.mark_evaluation(fp, pdf_file.name)
    ...
    try:
        ...
        st.error(
            f"The extracted items were not valid XML: {e}\n\n"
            "The extraction itself was saved to intermediate/extracted_items.xml — fix the offending line there and re-run, or start the evaluation again."
        )
        return
...
def render_generate_panel(fp: dict, slug: str) -> None:
    ...
    if err:
        ...
        debug_name = ("p3_debug.xml" if err["phase"] == "p3"
                      else f"{err['phase']}_debug.txt")
        st.caption(f"If a raw response was saved it is in `intermediate/{debug_name}`.")
