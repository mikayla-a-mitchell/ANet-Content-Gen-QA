...
def strip_latex(s: str) -> str:
    ...
    # Comparison commands -> plain symbols. Done as one alternation, longest
    # first, with a lookahead so "\le" cannot match the start of "\leq".
    # Two bugs lived here: plain .replace() turned "\leq" into "<=q", and
    # "\lt"/"\gt" were not listed at all, so the generic command-stripper
    # below deleted them — which made "w \lt 25" and "w \gt 25" both reduce
    # to "w 25" and produced a false "duplicate choices" finding on perfectly
    # good inequality items.
    _CMP = {"leq": "<=", "geq": ">=", "neq": "!=", "le": "<=", "ge": ">=",
            "ne": "!=", "lt": "<", "gt": ">", "approx": "~="}
    out = re.sub(r"\\(leq|geq|neq|approx|le|ge|ne|lt|gt)(?![a-zA-Z])",
                 lambda m: _CMP[m.group(1)], out)
...
def check_latex(q: dict) -> list:
    ...
    for field, text in fields:
        ...
        for m in re.finditer(r"\\\((.+?)\\\)", text, re.DOTALL):
            ...
            if re.search(r"[<>]", body):
                fixed = (body.replace("<=", r"\le ").replace(">=", r"\ge ")
                             .replace("<", r"\lt ").replace(">", r"\gt "))
                fixed = re.sub(r"[ \t]+", " ", fixed).strip()
                out.append(_finding(
                    item, "Comparison symbol uses its command form", SEV_HIGH,
                    "A bare < or > inside LaTeX is invalid XML and vanishes from the "
                    "rendered page because a browser reads it as a tag. Use \\lt, \\gt, "
                    "\\le, \\ge or \\neq.",
                    field, text, text.replace(body, fixed)))
...
# ── XML sanitisation for model output ─────────────────────────────────────────
# A bare "<" is illegal as XML text, and math items about comparison are full of
# them: an answer choice of "\(<\)" is both legitimate content and a parse
# error. It breaks twice over — ElementTree refuses the document, and even if it
# didn't, render.py drops that text straight into HTML where a browser reads
# "<\)" as the start of a tag and the symbol disappears from the page.
#
# So comparison symbols inside math are rewritten to their LaTeX command form,
# which KaTeX renders identically and which survives both XML and HTML intact.
# Anything still stray afterwards is escaped so the document always parses.

_MATH_SPAN = re.compile(r"\\\((.+?)\\\)", re.DOTALL)
_ENTITY = re.compile(r"&(?:#\d+|#x[0-9A-Fa-f]+|[A-Za-z][A-Za-z0-9]*);")
_REAL_TAG = re.compile(r"<(?:[!?]|/?[A-Za-z_][\w.:-]*(?:\s+[^<>]*?)?/?>)")

# Longest first: "&lt;=" must be consumed before "&lt;", and "<=" before "<".
# Both the raw character and its XML entity are handled, because a model may
# emit either. The entity form parses as valid XML but still decodes back to a
# bare "<" that disappears from the rendered HTML, so it needs the same fix.
_MATH_REPLACEMENTS = [
    ("&lt;=", r"\le "), ("&gt;=", r"\ge "),
    ("&lt;", r"\lt "), ("&gt;", r"\gt "),
    ("<=", r"\le "), (">=", r"\ge "), ("!=", r"\neq "),
...
def _normalize_math_comparisons(span: str) -> str:
    """Rewrite bare comparison characters inside one math span.

    Returns the span untouched when there is nothing to rewrite. That matters
    because this runs on every read: an earlier version collapsed whitespace
    in every span it saw, so merely opening a clean lesson rewrote its files
    and churned content nobody had asked to change."""
    if not re.search(r"[<>]|!=|&lt;|&gt;", span):
        return span
    lead = span[:len(span) - len(span.lstrip())]
    trail = span[len(span.rstrip()):]
    core = span.strip()
    for bare, cmd in _MATH_REPLACEMENTS:
        core = core.replace(bare, cmd)
    return lead + re.sub(r"[ \t]+", " ", core).strip() + trail


def _rewrite_math_span(m: re.Match) -> str:
    """Normalize one \\(...\\) span, unless it is not really one.

    `_MATH_SPAN` pairs each "\\(" with the next "\\)", so a span the model
    left unclosed runs on until some *later* item's closer and swallows the
    markup in between. Rewriting that as math turned real tags into "\\lt
    /STEM\\gt" — the document stopped parsing and the item's text was
    destroyed on the way. A span that contains a real tag is therefore left
    exactly as it was found: the stray "\\(" is a content problem for a human
    to see, not a licence to edit the structure around it."""
    inner = m.group(1)
    if _REAL_TAG.search(inner):
        return m.group(0)
    return "\\(" + _normalize_math_comparisons(inner) + "\\)"


def _escape_stray(s: str, char: str, entity: str, keep: re.Pattern) -> str:
    out, i = [], 0
    while True:
        j = s.find(char, i)
        if j < 0:
            ...
            return "".join(out)
        out.append(s[i:j])
        out.append(char if keep.match(s, j) else entity)
        i = j + 1


def sanitize_item_xml(xml_text: str) -> str:
    """Make model-produced item XML well-formed without changing its meaning.

    Safe to run on text that is already clean, and safe to run twice. Applied
    both when a phase's output is written and when a saved file is read, so an
    artifact produced before this existed is repaired on load rather than
    needing to be regenerated."""
    if not xml_text:
        return xml_text

    # CDATA (hand-written SVG) is verbatim by definition — set it aside.
    shelf = []

    def _stash(m):
        shelf.append(m.group(0))
        return f"\x00CDATA{len(shelf) - 1}\x00"

    text = re.sub(r"<!\[CDATA\[.*?\]\]>", _stash, xml_text, flags=re.DOTALL)

    text = _MATH_SPAN.sub(_rewrite_math_span, text)
    text = _escape_stray(text, "&", "&amp;", _ENTITY)
    text = _escape_stray(text, "<", "&lt;", _REAL_TAG)

    for i, original in enumerate(shelf):
        text = text.replace(f"\x00CDATA{i}\x00", original)
    return text
