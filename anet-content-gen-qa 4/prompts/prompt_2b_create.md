...
- **CRITICAL — comparison symbols use their command form, never the bare character.**
  Write `\lt`, `\gt`, `\le`, `\ge`, `\neq` — not `<`, `>`, `<=`, `>=`, `!=`. A bare `<`
  is not legal XML text, so it breaks the pipeline before the item ever renders, and it also
  disappears from the HTML page because a browser reads it as the start of a tag. So an
  answer choice of "less than" is `\( \lt \)` and an inequality is `\( 3 \lt n \lt 8 \)`.
  Ampersands in text are `&amp;` for the same reason.
