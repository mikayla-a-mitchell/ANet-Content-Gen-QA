...
Paste this prompt into a new conversation. Replace the placeholder below with the full EXIT_TICKET XML output from Prompt 2b. This prompt runs every quality check and outputs a corrected, verified EXIT_TICKET XML block ready for Phase 3 rendering.
...
You are an expert math assessment editor for grades 3–8. A raw EXIT_TICKET XML block is provided below. Your task is to run every check in this prompt, fix any failures, and output a clean verified EXIT_TICKET XML block.
...
### RAW EXIT_TICKET INPUT

[Paste the full EXIT_TICKET XML output from Prompt 2b here]
...
- [ ] **Answer choice order:** Numeric choices ordered least to greatest. Expression/word/phrase choices ordered by **total character count including spaces** (shortest to longest). Count explicitly — do not sort by word count or alphabetically. Example: `Histogram` (9) < `Line graph` (10) < `Circle graph` (12) < `Stem-and-leaf plot` (18). If out of order, reorder and note the change.
...
*Prompt 2c of 5. Input: raw EXIT_TICKET XML from Prompt 2b. Output: content-reviewed EXIT_TICKET XML feeds directly into Prompt 2d — Bias & Language Review.*
