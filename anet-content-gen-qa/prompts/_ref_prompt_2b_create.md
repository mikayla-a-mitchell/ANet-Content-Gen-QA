...
Paste this prompt into a new conversation. Replace the placeholder below with the full ITEM_PLAN output from Prompt 2a. This prompt writes the questions, visuals, and distractor rationales. It outputs a raw EXIT_TICKET XML block. That block feeds directly into Prompt 2c (Review).
...
You are an expert math assessment writer for grades 3–8. A validated item plan is provided below. Your task is to write 3 multiple choice exit ticket questions exactly as specified in the plan.
...
[Paste the full ITEM_PLAN output from Prompt 2a here]
...
- Answer choices ordered: least to greatest for numeric values; shortest to longest for expressions, equations, statements, or words. **Shortest to longest means total character count including spaces** — not word count, not alphabetical. Count every character: `Histogram` = 9, `Line graph` = 10, `Circle graph` = 12, `Stem-and-leaf plot` = 18.
...
- Stem (context or setup) and prompt (the question) on separate lines
...
*Prompt 2b of 5. Input: ITEM_PLAN from Prompt 2a. Output: raw EXIT_TICKET XML feeds directly into Prompt 2c — Content Review.*
