...
Work through all five steps in order. **Do that work internally and do not narrate it.**
Your entire response is the single `<ITEM_PLAN>` block specified at the end of this prompt —
no preamble, no step-by-step write-up, no closing summary, nothing outside the block.

This matters for a practical reason: the app keeps only the `<ITEM_PLAN>` block and discards
everything else in your response. A narrated version of the five steps restates what the
block already carries, so writing it costs minutes of generation time and is then thrown
away. Every check below is still binding — each one records its result in a field of the
block, so the reasoning is preserved where it is actually read.

Do not write any question text or SVG code here; that happens in Prompt 2b. This is a plan.
...
**For each construct, work out the following — these become the `<CONSTRUCT>`,
`<SOURCE_GOAL>`, `<LESSON_EVIDENCE>` and `<SCOPE>` fields of that item in the block:**
...
**Hard gate — fill this table internally before proceeding to Step 2, and record it in
`<SET_CHECKS><GOAL_COVERAGE>` in the output block:**
...
**Quality-of-coverage check — required after filling the table. One clause per item in
`<SET_CHECKS><GOAL_DEPTH>`:**
...
Work out the lesson's teaching arc and confirm each construct maps to a different phase of
it. If all 3 address the same phase, revise the selection before proceeding. Record the
outcome as one sentence in `<SET_CHECKS><TEACHING_ARC>` — the arc and which item sits where,
not a paragraph about it.



**Visual type distribution check — required before finalising construct selection. Record
in `<SET_CHECKS><VISUAL_SPREAD>` as one line:**
...
**For each misconception, work out the following — these become the `<ERROR_CATEGORY>`,
`<REASONING>`, `<WRONG_ANSWER>` and `<DR>` fields of that misconception in the block:**
...
For every distractor, verify that following the reasoning in the DR produces the exact value in the AC. If the DR reasoning would produce a different value, revise one or both until they match precisely. Record each result in that misconception's `<ACDR_CHECK>` field — PASS plus the arithmetic that proves it, in one line.
...
For each question, settle all 4 answer choices (correct answer + 3 distractors) and confirm
every check below internally. Resolve every flag before moving to Step 4. The choices
themselves go in `<ANSWER_CHOICES>`; do not write out a per-check PASS list.
...
**Difficulty consistency across the set — required before finalising Step 3. Record the
tiers and verdict in `<SET_CHECKS><DIFFICULTY>` as one line:**
...
**Same correct answer check — required for all categorical-choice items. Record in
`<SET_CHECKS><SAME_ANSWER>`:**
...
**Consistent-distractor pattern check — required for all 3-item sets. Record in
`<SET_CHECKS><DISTRACTOR_PATTERN>`:**
...
For each question without a visual, put the one-sentence reason in that item's
`<VISUAL_PLAN><SPEC>` field.
...
For each planned question, check every item below internally. Resolve every flag before
outputting the block. Record the result per item in that item's `<BIAS_REVIEW>` field: the
word PASS on its own if everything passed, or the flags you found and how you resolved them.
Do not list the checks that passed.
...
<SET_CHECKS>
<GOAL_COVERAGE>[One line per item: item number | construct in <=12 words | learning goal covered. If the lesson has one stated goal, say so once — that is not a diversity failure.]</GOAL_COVERAGE>
<GOAL_DEPTH>[One clause per item confirming it tests the goal at the depth the lesson teaches it.]</GOAL_DEPTH>
<TEACHING_ARC>[One sentence: the arc, and which item sits at which phase.]</TEACHING_ARC>
<VISUAL_SPREAD>[One line: the visual type per item, and whether that spread is acceptable.]</VISUAL_SPREAD>
<DIFFICULTY>[One line: the value tier per item, and the verdict.]</DIFFICULTY>
<SAME_ANSWER>[PASS — correct answers are [list] | REVISED — what changed | N/A — not categorical-choice items]</SAME_ANSWER>
<DISTRACTOR_PATTERN>[PASS — no option is a distractor in 2+ items without ever being correct | REVISED — what changed]</DISTRACTOR_PATTERN>
</SET_CHECKS>

</ITEM_PLAN>
```

Output the block and nothing else. Any text before `<ITEM_PLAN>` or after `</ITEM_PLAN>` is
discarded by the app, so it costs generation time and reaches no one.
